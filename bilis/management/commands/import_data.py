...
class Command(BaseCommand):
    ...
    def import_players(self, filename="bilis_players.csv"):
        ...
        for row in reader:
            ...
            p = Player.objects.create(
                ...
                elo=elo,
                fargo=fargo,
                favorite_color=randint(0, 16711680),
            )
            p.save()
    ...
    def import_games(self, filename="bilis_games.csv"):
            ...
            try:
                dt = timezone.make_aware(
                    datetime.fromtimestamp(
                        mktime((strptime(datetimestr, "%m/%d/%Y %I:%M:%S %p")))
                    ),
        ...
        print(
            "Successfully processed {rows_out}, rejected {rows_rejected}".format(
                rows_out=rows_out, rows_rejected=rows_rejected
            )
        )
