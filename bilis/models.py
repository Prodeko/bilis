...
class Player(models.Model):
    ...
    def get_first_game_datetime(self):
        if self.won_games.exists():
            won_games_first_datetime = self.won_games.all().aggregate(
                models.Min("datetime")
            )["datetime__min"]
        else:
            won_games_first_datetime = timezone.now()
        if self.lost_games.exists():
            lost_games_first_datetime = self.lost_games.all().aggregate(
                models.Min("datetime")
            )["datetime__min"]
        else:
    ...
    def get_last_game_datetime(self):
        if self.won_games.exists():
            won_games_last_datetime = self.won_games.all().aggregate(
                models.Max("datetime")
            )["datetime__max"]
        else:
            won_games_last_datetime = timezone.now()
        if self.lost_games.exists():
            lost_games_last_datetime = self.lost_games.all().aggregate(
                models.Max("datetime")
            )["datetime__max"]
        else:
    ...
    def get_games_per_day(self):
        days_between_first_and_last = (
            self.get_last_game_datetime().date()
            - self.get_first_game_datetime().date()
            + timedelta(1)
        )
        return "{:.2}".format(
            self._get_games_count() / days_between_first_and_last.days
        )
    ...
    def is_active(self):
        ...
        if timezone.now().date() - self.get_last_game_datetime().date() > timedelta(
            100
        ):
    ...
    def __str__(self):
        return "#{id} {name} ({rating})".format(
            id=self.pk, name=self.name, rating=self.fargo
        )
    ...
    def update_rating_fargo(self, opponent_rating, opponent_games, result):
        ...
        if result > 0:
            change = (
                630
                * (1 - (1 / (1 + math.pow(2, ((opponent_rating - self.fargo) / 100)))))
                * ((opponent_robust - 1) / (self_robust * opponent_robust))
            )
        else:
            change = (
                630
                * (0 - (1 / (1 + math.pow(2, ((opponent_rating - self.fargo) / 100)))))
                * ((opponent_robust - 1) / (self_robust * opponent_robust))
            )
...
class Game(models.Model):
    ...
    loser = models.ForeignKey(
        Player, related_name="lost_games", on_delete=models.CASCADE
    )
